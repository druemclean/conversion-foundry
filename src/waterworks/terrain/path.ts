...
/**
 * Heading of the polyline at the point nearest (x, z), as a Y-rotation.
 *
 * Returns `atan2(dx, dz)` of the local tangent, which is the rotation that
 * maps a mesh's local +Z onto the flow direction — so a board whose width
 * runs along local X ends up spanning the channel. Same convention the kerb
 * stones use.
 */
export function tangentAngle(pts: Vec2[], x: number, z: number): number {
  ...
  const prev = pts[Math.max(0, nearest - 1)];
  const next = pts[Math.min(pts.length - 1, nearest + 1)];
